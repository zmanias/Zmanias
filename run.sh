...
then
    ...
    if [ $pil = 2 ] || [ $pil = 02 ]
    then
	echo $white"["$green"√"$white"]"$white" Open WhatsApp"
	sleep 2
	xdg-open https://api.whatsapp.com/send/?phone=6283840600405&text=Assalamualaikum+kak,+boleh+minta+username+dan+password+tools+nya?&app_absent=0
        sh run.sh
    fi
    if [ $pil = 3 ] || [ $pil = 03 ]
    then
